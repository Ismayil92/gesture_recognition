...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, sample_size=224, width_mult=1.):
        ...
        #LSTM layers
        self.lstm_hidden_size = 1280
        
        self.lstm_layers = nn.LSTM(self.last_channel, self.lstm_hidden_size, 2, dropout=0.2)
        # building classifier
        self.classifier = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(self.lstm_hidden_size*8, num_classes, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.features(x)                 #print(x.data.size()[-2:])
        x = F.avg_pool3d(x, (1,4,4))    
        x = x.permute(2,0,1,3,4)
        x = x.view(x.size(0), x.size(1), -1)   
        #h0 = torch.ones(2, 32, self.lstm_hidden_size).cuda()
        #c0 = torch.ones(2, 32, self.lstm_hidden_size).cuda()           
